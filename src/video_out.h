...
void video_init(int samples_per_cc, int machine, const uint32_t* palette, int ntsc)
{
    ...
    _line_width = NTSC_COLOR_CLOCKS_PER_SCANLINE * samples_per_cc;  // Keep this as is
    _line_count = NTSC_LINES;
    _hsync_long = usec(63.555 - 4.7);

    // Slightly increase the offset for horizontal compression
    _active_start = usec((samples_per_cc == 4 ? 10 : 10.5) - (1.0 / (_line_width / NTSC_COLOR_CLOCKS_PER_SCANLINE)));
